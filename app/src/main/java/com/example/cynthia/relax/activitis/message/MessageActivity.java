...
public class MessageActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try{
                    socket = new Socket("192.168.1.108", 10010);
        ...
        send.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                final String data = inputText.getText().toString();
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        try {
        ...
        public void handleMessage(Message msg) {
            ...
            if(msg.what == 1){
                ...
                if(split[0].equals(localPort+"")){
                    MessageBean bean = new MessageBean(split[1],1, split[2]);
                    msgList.add(bean);
                }
                else{
                    MessageBean bean = new MessageBean(split[1], 2, split[2]);
                    msgList.add(bean);
                }
                //向适配器set数据
